...
QuantLib::Period QuantLibAddin::getPeriod(const std::string &in) {
    ...
    //ret.normalize();
    // In the old build the functionality below was implemented in the ctor of QuantLibAddin::Euribor.
...
template struct QuantLibAddin::GetObject<QuantLibAddin::Leg, QuantLib::Leg>;

// FIXME this QuantLib(Addin)-specific behavior belongs here in the QuantLibAddin project,
// but the template should be in the reposit namespace, with a local specialization here.
boost::shared_ptr<QuantLib::YieldTermStructure> QuantLibAddin::GetObject<QuantLibAddin::YieldTermStructure, QuantLib::YieldTermStructure>::f(const std::string &in) {
    RP_GET_OBJECT(temp, in, reposit::Object)
    return QuantLibAddin::CoerceLibrarySame<
            QuantLibAddin::YieldTermStructure,
            QuantLib::YieldTermStructure>()(temp);
}
