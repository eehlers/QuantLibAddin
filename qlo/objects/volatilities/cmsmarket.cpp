...
#include <qlo/objects/volatilities/cmsmarket.hpp>
...
namespace QuantLibAddin {
    ...
    CmsMarket::CmsMarket(
        ...
        bool permanent)
    : reposit::LibraryObject<QuantLib::CmsMarket>(properties, permanent)
    {
    ...
    vector<vector<reposit::property_t> > CmsMarket::getCmsMarket()
    {
    ...
    vector<vector<reposit::property_t> > browseCmsMarket(const Matrix& cmsMarket) {
        Size numberOfColumn = 14;
        Size numberOfRows = cmsMarket.rows()+1;
        vector<vector<reposit::property_t> > result(numberOfRows, vector<reposit::property_t>(numberOfColumn));
