...
namespace QuantLibAddin {
    ...
    vector<Date> IMMNextDates(const Date& date,
    ...
    vector<string> IMMNextCodes(const Date& date,
                                  const vector<bool>& mainCycle) {
    ...
    vector<Date> ASXNextDates(const Date& date,
                                const vector<bool>& mainCycle) {
        QL_REQUIRE(!mainCycle.empty(), "Main cycle flags vector is empty.");
        Date d = (date == Date() ?
                  Date(Settings::instance().evaluationDate()) :
    ...
    vector<string> ASXNextCodes(const Date& date,
                                  const vector<bool>& mainCycle) {
