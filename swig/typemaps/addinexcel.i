...
%typemap(rp_tm_xll_cnv) std::vector<QuantLib::Natural> const & %{
...
%typemap(rp_tm_xll_cnv) boost::shared_ptr<QuantLibAddin::RateHelper> const & qlarh %{
...
%typemap(rp_tm_xll_cnv) std::vector<QuantLib::Date> const & %{
...
%typemap(rp_tm_xll_cnv) std::vector<boost::shared_ptr<QuantLibAddin::RateHelper> > const & %{
...
%typemap(rp_tm_xll_cnv) std::vector<boost::shared_ptr<QuantLib::RateHelper> > const & %{
...
%typemap(rp_tm_xll_cnv) std::vector<QuantLib::Handle<QuantLib::Quote> > const & %{
        std::vector<reposit::property_t> $1_name_vec =
            reposit::operToVector<reposit::property_t>(*$1_name, "$1_name");
...
%typemap(rp_tm_xll_cnv) std::vector<boost::shared_ptr<QuantLibAddin::Leg> > const & %{
        std::vector<std::string> $1_name_vec =
            reposit::operToVector<std::string>(*$1_name, "$1_name");
...
%typemap(rp_tm_xll_cnv) std::vector<QuantLib::Leg> const & %{
        std::vector<std::string> $1_name_vec =
            reposit::operToVector<std::string>(*$1_name, "$1_name");
...
%typemap(rp_tm_xll_arg) QuantLib::Period const & "$1_name_cnv";
%typemap(rp_tm_xll_arg) ql_tp_handle "$1_name_handle";
